...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        funcs = df["function_before"].tolist()
...
def convert_examples_to_features(func, label, tokenizer, args):
    if args.use_word_level_tokenizer:
        ...
        if len(encoded) > args.block_size - 2:
            encoded = encoded[:args.block_size - 2]
        encoded.insert(0, 0)
        encoded.append(2)
        if len(encoded) < args.bloack_size:
            padding = args.bloack_size - len(encoded)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1'] > best_f1:
                        ...
                        output_dir_ckpt = os.path.join(args.output_dir, checkpoint_prefix)
                        os.makedirs(output_dir_ckpt, exist_ok=True)

                        checkpoint_path = os.path.join(output_dir_ckpt, args.model_name)
                        ...
                        logger.info("Saving full checkpoint to %s", checkpoint_path)
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    probs = logits[:, 1]  # probability for positive class
    correctness = (y_preds == y_trues)

    result_df = pd.DataFrame({
        "true_label": y_trues,
        "pred_label": y_preds.astype(int),
        "probability": probs,
        "correct": correctness
    })

    # Separate correct and incorrect predictions (optional)
    correct_df = result_df[result_df["correct"] == True]
    incorrect_df = result_df[result_df["correct"] == False]

    # Create output directory
    os.makedirs(args.output_dir, exist_ok=True)

    # Save CSVs
    result_df.to_csv(os.path.join(args.output_dir, f"test_results_{args.model_name}.csv"), index=False)
    correct_df.to_csv(os.path.join(args.output_dir, f"test_correct_{args.model_name}.csv"), index=False)
    incorrect_df.to_csv(os.path.join(args.output_dir, f"test_incorrect_{args.model_name}.csv"), index=False)

    logger.info(f"Saved detailed results to {args.output_dir}")
...
def generate_result_df(logits, y_trues, y_preds, args):
    ...
    all_num_lines = []
    all_function_before = df["function_before"].tolist()
    for func in all_function_before:
    ...
    return pd.DataFrame({"logits": logits, "y_trues": y_trues, "y_preds": y_preds, 
                         ...
                         "flaw_line": df["flaw_line"], "function_before": df["function_before"]})
    ...
    if do_explanation:
        if reasoning_method == "attention":
            ...
            if reasoning_method == "deeplift":
                baselines = torch.zeros(1, args.block_size, 768, requires_grad=True).to(args.device)
                reasoning_model = DeepLift(model)
            elif reasoning_method == "deeplift_shap":
                baselines = torch.zeros(16, args.block_size, 768, requires_grad=True).to(args.device)
...
def main():
    ...
    ## parameters
    parser.add_argument("--train_data_file", default="D:/AVD/LineVul-main/data/big-vul_dataset/train_rijha_updated.csv", type=str, required=False,
    ...
    parser.add_argument("--eval_data_file", default="D:/AVD/LineVul-main/data/big-vul_dataset/val_rijha_updated.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--test_data_file", default="D:/AVD/LineVul-main/data/big-vul_dataset/test_rijha_updated.csv", type=str,
                        help="An optional input evaluation data file to evaluate the perplexity on (a text file).")
    parser.add_argument("--model_name", default="12heads_linevul_model_codebert_123.bin", type=str,
    ...
    log_file = os.path.join(log_dir, "training_unixcoder-base-seed42.log")
    ...
    # ✅ Overwrite weights with your checkpoint if it exists
    checkpoint_path = os.path.join(args.output_dir, "checkpoint-best-f1", args.model_name)
    if os.path.exists(checkpoint_path):
        ...
        # Optional: load optimizer, scheduler, epoch, global_step if you are resuming training
        start_epoch = checkpoint.get("epoch", 0)
        best_f1 = checkpoint.get("best_f1", 0)

        logger.info(f"Loaded checkpoint from {checkpoint_path}, starting at epoch {start_epoch}, best_f1={best_f1}")
    else:
        logger.warning(f"Checkpoint not found at {checkpoint_path}, using base model only.")
        start_epoch = 0
        best_f1 = 0
